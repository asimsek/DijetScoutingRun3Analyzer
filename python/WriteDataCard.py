...
import ROOT as rt
from rootTools import RootIterator, Utils
...
def fixPars(w, label, doFix=True, setVal=None):
    parSet = w.allVars()
    for par in RootIterator.RootIterator(parSet):
        if label in par.GetName():
            par.setConstant(doFix)
            if setVal is not None: par.setVal(setVal)

def initializeWorkspace(w,cfg,box,scaleFactor=1.,penalty=False,multi=False,x=None,emptyHist1D=None,printWorkspace=True):
...
def applyTurnonFunc(hist,effFr,w):
    ...
    for p in RootIterator.RootIterator(effFr.floatParsFinal()):
...
if __name__ == '__main__':
    ...
    if options.inputFitFile is not None:
        ...
        for p in RootIterator.RootIterator(frIn.floatParsFinal()):
        ...
        for p in RootIterator.RootIterator(frIn.constPars()):
            if w.var(p.GetName()) != None:
                w.var(p.GetName()).setVal(p.getVal())
                w.var(p.GetName()).setError(p.getError())
        ...
        if options.deco or options.refit:
            ...
            paramsToDecoNames = []
            for p in RootIterator.RootIterator(frSpB.floatParsFinal()):
