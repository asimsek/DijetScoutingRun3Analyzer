...
def importToWS(workspace, *args):
        ...
        # workaround for ROOT6.02 PyROOT issue: https://root.cern.ch/phpBB3/viewtopic.php?t=19382
        import ROOT as rt
        if args[0].InheritsFrom('RooAbsArg') or args[0].InheritsFrom('RooArgSet') or args[0].InheritsFrom('RooAbsData'):
            dummyargs = args + (rt.RooCmdArg(),)
            return getattr(workspace,'import')(*dummyargs)
        else:
